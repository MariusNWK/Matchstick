...
void apply_ai_algo_and_update_board(char **board, dimensions_t dim,
int limit, int player)
{
    list_t *library = define_lines(board, dim);
    int nb_matches = find_nb_matches_to_remove(board, dim, define_limit(library, limit));
    int line = 0;
    ...
    if (player == 1) {
        ...
        remove_msg("Player 1", nb_matches, line);
    }
    else {
        my_putstr("Player 2 turn...\n");
        remove_msg("Player 2", nb_matches, line);
    }
    */
